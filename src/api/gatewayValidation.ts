...
export async function validarTokenConGateway(token: string): Promise<void> {
    ...
    try {
        ...
        debugLog("gateway-validation", "Error de red al validar con gateway", {
            networkError,
        });
        throw new Error(
            "No se pudo conectar con el gateway para validar la sesión.",
        );
    }

    debugLog("gateway-validation", "Respuesta del gateway", {
